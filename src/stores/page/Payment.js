...
reducer.register('PAYMENT_DATA', (state, action) => {
  let {data} = action.params;
  state.doc = data;
  return state;
});

reducer.register('PAYMENT_DATA_ITEM', (state, action) => {
  let {data} = action.params;
  state.doc.data = data;
  return state;
});

reducer.register('PAYMENT_SELECTED', (state, action) => {
  let {data} = action.params;
  state.manage = data;
  return state;
});

reducer.register('PAYMENT_CONTENT', (state, action) => {
  let {data} = action.params;
  state.doc.data.list = data;
  return state;
});

reducer.register('PAYMENT_CONTENT_ITEM', (state, action) => {
